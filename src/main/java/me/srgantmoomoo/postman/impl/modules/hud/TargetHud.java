...
public class TargetHud extends HudModule {
    ...
    @EventHandler
    private final Listener<RenderEntityNameEvent> OnDamageBlock = new Listener<>(Event::cancel);
    ...
    private class TargetHUDComponent extends HUDComponent {
        ...
        public void render(Context context) {
            ...
            if (mc.world != null && mc.player.ticksExisted >= 10) {
                EntityPlayer entityPlayer = (EntityPlayer) mc.world.loadedEntityList.stream().filter(TargetHud.this::isValidEntity).map(entity -> (EntityLivingBase) entity).min(Comparator.comparing(c -> mc.player.getDistance(c))).orElse(null);
